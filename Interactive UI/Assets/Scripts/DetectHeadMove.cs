...
public class DetectHeadMove : MonoBehaviour
{
    ...
    public int CheckMovement()
    {
        ...
        for (int i = 0; i < 80; i++)
        {
            if (angles[i].x < centerAngle.x - 1.0f && !up)
            {
                up = true;
                print("up");
            }
            else if (angles[i].x > centerAngle.x + 1.0f && !down)
            {
                down = true;
                                print("down");
            ...
            if (angles[i].y < centerAngle.y - 1.0f && !left)
            {
                left = true;
                                print("left");

            }
            else if (angles[i].y > centerAngle.y + 1.0f && !right)
            {
                right = true;
                                print("right");
